...
def cluster_archetypes(features):
    ...
    # name clusters by centroid characteristics, not by arbitrary index
    ...
    all_counts = [c for v in check_counts_by_cluster.values() for c in v]
    median_checks = float(np.median(all_counts))

    centroid_names = {}
    for label in range(k):
        avg_day = np.mean(day_by_cluster[label])
        avg_checks = np.mean(check_counts_by_cluster[label])
        if avg_day >= 5:
            centroid_names[label] = "Weekend Planner"
        elif avg_checks >= median_checks:
            centroid_names[label] = "Frequent Monitor"
        else:
